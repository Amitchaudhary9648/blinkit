...
const calculateControlPoint = (p1, p2) => {
    ...
    const xc = xm + (h * (p2[1] - p1[1]) / (2 * Math.sqrt((p2[0] - p1[0]) ** 2 + (p2[1] - p1[1]) ** 2))) * (w / d);
    const yc = ym + (h * (p2[0] - p1[0]) / (2 * Math.sqrt((p2[0] - p1[0]) ** 2 + (p2[1] - p1[1]) ** 2))) * (w / d);

    const controlPoint = [xc, yc];
    return controlPoint;
};
