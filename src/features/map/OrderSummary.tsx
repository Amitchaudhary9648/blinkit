...
const OrderSummary:FC<{order: any}> = ({order}) => {
    const totalPrice = order?.items?.reduce((total: any, cartItem: any) => total + cartItem.item.price * cartItem.count, 0) || 0
    return (
                    ...
                    <View style={styles.flexRow} key={index}>
                        ...
                        <View style={{width: '20%', alignItems: 'flex-end'}}>
                            <CustomText 
                                ...
                                style={{alignSelf: 'flex-end', marginTop: 4 }}>₹{item?.count * item?.item?.price}</CustomText>
                            <CustomText
                                variant='h8' 
                                fontFamily={Fonts.Medium}
                                style={{alignSelf: 'flex-end', marginTop: 4 }}>{item?.count}x</CustomText>
                        </View>
                    </View>
                )
            })}
